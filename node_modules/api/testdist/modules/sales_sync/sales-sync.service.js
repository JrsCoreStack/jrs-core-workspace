...
var SalesSyncService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.SalesSyncService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const config_1 = require("@nestjs/config");
const sales_sync_status_entity_1 = require("./entities/sales-sync-status.entity");
const financial_posting_service_1 = require("../financial_posting/financial_posting.service");
const financial_entry_entity_1 = require("../financial_entry/entities/financial_entry.entity");
const products_config_1 = require("./config/products.config");
let SalesSyncService = SalesSyncService_1 = class SalesSyncService {
    syncStatusRepository;
    financialEntryRepository;
    dataSource;
    financialPostingService;
    configService;
    logger = new common_1.Logger(SalesSyncService_1.name);
    productDataSources = new Map();
    productsConfig = [];
    constructor(syncStatusRepository, financialEntryRepository, dataSource, financialPostingService, configService) {
    ...
    async onModuleInit() {
        ...
        this.productsConfig = (0, products_config_1.getProductsConfig)(this.configService).filter((config) => {
            const hasConfig = config.database.host &&
                config.database.port &&
                config.database.username &&
                config.database.password &&
                config.database.database;
            if (!hasConfig) {
                this.logger.warn(`⚠️  Configuração incompleta para ${config.productName}. Produto será ignorado.`);
            }
            return hasConfig;
        });
        for (const productConfig of this.productsConfig) {
            try {
                ...
                this.logger.log(`✅ Conexão com banco ${productConfig.productName} estabelecida`);
            }
            catch (error) {
                this.logger.error(`❌ Erro ao conectar com banco ${productConfig.productName}: ${error.message}`);
            }
        }
        ...
        try {
            ...
            for (const productConfig of this.productsConfig) {
                ...
                for (const saleTypeConfig of productConfig.saleTypes) {
                    ...
                    syncPromises.push(this.syncSalesByType(productConfig, saleTypeConfig, syncType, dataSource));
                }
            }
     ...
     */
    async syncSalesByType(productConfig, saleTypeConfig, syncType, dataSource) {
        try {
            ...
            const sales = await this.fetchNewSalesFromDatabase(syncStatus, saleTypeConfig, dataSource);
            if (sales.length === 0) {
                this.logger.debug(`Nenhuma venda nova encontrada para ${productConfig.productName} - ${saleTypeConfig.referenceType}`);
                return;
            }
            this.logger.log(`📦 [${productConfig.productName}] Encontradas ${sales.length} vendas novas para ${saleTypeConfig.referenceType}`);
            ...
            for (const sale of sales) {
                try {
                    ...
                    if (existingCount > 0) {
                        this.logger.log(`⏭️  [${productConfig.productName}] Venda ${sale.reference_id} já processada (${existingCount} entries encontrados), pulando...`);
                        lastId = Math.max(lastId, sale.id);
                        continue;
                    }
                    this.logger.debug(`🔄 [${productConfig.productName}] Processando venda ${sale.reference_id} (${saleTypeConfig.referenceType})...`);
                    // Processar venda
                    ...
                    if (savedCount === 0) {
                        this.logger.warn(`⚠️  [${productConfig.productName}] Venda ${sale.reference_id} processada mas nenhum entry foi criado!`);
                    }
                    else {
                        this.logger.debug(`✅ [${productConfig.productName}] Venda ${sale.reference_id} processada com sucesso (${savedCount} entries criados)`);
                    }
                    processed++;
                    lastId = Math.max(lastId, sale.id);
                    ...
                    this.logger.error(`❌ [${productConfig.productName}] Erro ao processar venda ${sale.reference_id}: ${error.message}`, error.stack);
            ...
            if (sales.length > 0) {
                ...
                await this.syncStatusRepository.save(syncStatus);
                this.logger.debug(`📝 Status atualizado: last_synced_id=${lastId}, total_synced=${syncStatus.total_synced}`);
                this.logger.log(`✅ [${productConfig.productName}] ${processed} vendas processadas para ${saleTypeConfig.referenceType}. Total: ${syncStatus.total_synced}`);
            }
        }
        catch (error) {
            this.logger.error(`❌ Erro ao sincronizar ${productConfig.productName} - ${saleTypeConfig.referenceType}: ${error.message}`, error.stack);
            throw error;
        }
    }
    /**
     ...
     */
    async fetchNewSalesFromDatabase(syncStatus, saleTypeConfig, dataSource) {
        if (!dataSource?.isInitialized) {
            return [];
        }
        ...
        this.logger.debug(`🔍 Query SQL completa: ${query} | Parâmetros: [${params.join(', ')}]`);
        try {
            ...
            if (result.length > 0) {
                ...
                this.logger.debug(`🔍 Primeira linha retornada do banco: ID=${firstRow.id}, created=${createdDate}, year=${year}`);
                if (year !== 2026 && year !== 'N/A') {
                    this.logger.warn(`⚠️  ATENÇÃO: Venda de ${year} retornada! Query pode ter problema no filtro de ano.`);
                }
            }
            return result.map((row) => {
                ...
                if (sale.total_paid_amount === 0 && sale.ticket_amount === 0) {
                    this.logger.warn(`⚠️  Venda ${sale.reference_id} com valores zerados. Dados originais: ${JSON.stringify(row)}`);
                }
                return sale;
            });
        }
        catch (error) {
            this.logger.error(`❌ Erro ao buscar vendas do banco: ${error.message}`, error.stack);
            return [];
        }
    }
    /**
     ...
     */
    async syncSpecificSale(saleId, productId, referenceType) {
        try {
            // Encontrar configuração do produto
            const productConfig = this.productsConfig.find((config) => config.productId === productId);
            if (!productConfig) {
                return {
                    success: false,
            ...
            const saleTypeConfig = productConfig.saleTypes.find((type) => type.referenceType === referenceType);
            if (!saleTypeConfig) {
                return {
                    success: false,
            ...
            this.logger.error(`❌ Erro ao sincronizar venda ${saleId}: ${error.message}`, error.stack);
            return {
                success: false,
     ...
     */
    async fetchSaleById(saleId, saleTypeConfig, dataSource) {
        if (!dataSource?.isInitialized) {
            return null;
        }
        // Usar query customizada se fornecida, senão usar padrão
        ...
        query += ` LIMIT 1`;
        this.logger.debug(`🔍 Buscando venda específica: ID=${saleId} | Query: ${query.substring(0, 200)}...`);
        try {
            const result = await dataSource.query(query, params);
            if (result.length === 0) {
                return null;
            ...
            this.logger.error(`❌ Erro ao buscar venda ${saleId} do banco: ${error.message}`, error.stack);
            return null;
        }
    }
    /**
     ...
     */
    async processSale(sale, saleTypeConfig, productConfig) {
        // Log dos dados antes de processar
        this.logger.debug(`📊 Processando venda ${sale.reference_id}: total_paid=${sale.total_paid_amount}, ticket=${sale.ticket_amount}`);
        const saleData = {
        ...
        if (saleData.total_paid_amount === 0) {
            this.logger.warn(`⚠️  Venda ${sale.reference_id} tem total_paid_amount = 0. Dados: ${JSON.stringify(saleData)}`);
        }
        ...
        switch (saleTypeConfig.referenceType) {
            ...
            //   break;
            default:
                throw new Error(`Tipo de venda desconhecido: ${saleTypeConfig.referenceType}`);
        }
    }
    /**
     * Método manual para forçar sincronização (útil para testes)
     */
    async forceSync(productId, referenceType) {
        if (this.productDataSources.size === 0) {
        ...
        if (productId && referenceType) {
            ...
            const productConfig = this.productsConfig.find((p) => p.productId === productId);
            if (!productConfig) {
                throw new Error(`Produto não encontrado: ${productId}`);
            }
            const saleTypeConfig = productConfig.saleTypes.find((st) => st.referenceType === referenceType);
            if (!saleTypeConfig) {
                throw new Error(`Tipo de venda não encontrado: ${referenceType} para produto ${productId}`);
            }
            const dataSource = this.productDataSources.get(productId);
            if (!dataSource?.isInitialized) {
            ...
            return this.syncSalesByType(productConfig, saleTypeConfig, syncType, dataSource);
        }
        else if (productId) {
...
exports.SalesSyncService = SalesSyncService;
exports.SalesSyncService = SalesSyncService = SalesSyncService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(sales_sync_status_entity_1.SalesSyncStatusEntity)),
    __param(1, (0, typeorm_1.InjectRepository)(financial_entry_entity_1.FinancialEntryEntity)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository,
        typeorm_2.DataSource,
        financial_posting_service_1.FinancialPostingService,
