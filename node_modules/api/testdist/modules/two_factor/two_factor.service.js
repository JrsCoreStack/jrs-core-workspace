...
let TwoFactorService = class TwoFactorService {
    ...
    async generateSecret(email) {
        ...
        const otpauth = otplib_1.authenticator.keyuri(email, 'ERPOTicket', secret);
    ...
    async validateAccount({ code, secret, email, }) {
    ...
    async validateCode(code, email) {
        ...
        const isValid = await this.isCodeValid(code, user?.totp_secret);
        if (isValid) {
            return true;
        }
        else {
    ...
    async isCodeValid(code, secret) {
        const isValid = otplib_1.authenticator.check(code, secret);
        if (isValid) {
            return true;
        }
        return false;
